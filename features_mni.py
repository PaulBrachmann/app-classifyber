...
def compute_X_dm(
    superset, prototypes, distance_func=bundles_distances_mam, nb_points=20
):
    ...
    print(
        "Computing dissimilarity matrix (%s x %s)..." % (len(superset), len(prototypes))
    )
...
def compute_X_dm_local(
    superset, tract_name, distance_func=bundles_distances_mam, nb_points=20
):
    ...
    if distance_func == bundles_distances_mdf:
        ...
        superset = set_number_of_points(superset, nb_points)
    distance = partial(parallel_distance_computation, distance=distance_func)
    local_prot_fname = os.path.join(
        os.path.dirname(os.path.realpath(__file__)),
        "common_local_prototypes",
        "%s_common_prototypes.npy" % tract_name,
    )
    local_prototypes = np.load(local_prot_fname)
    print(
        "Computing dissimilarity matrix (%s x %s)..."
        % (len(superset), len(local_prototypes))
    )
...
def compute_X_roi(superset, tract_name):
    ...
    with open(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "IDs_tracts_dictionary.pickle"
        ),
        "rb",
    ) as f:  # python3
        u = pickle._Unpickler(f)
    ...
    if tractID < 37:
        roi_dir = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "templates_mni125"
        )
        ...
        roi_dir = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "templates_mni125_ICBM2009c"
        )
...
def compute_feature_matrix(
    ...
    if dm:
        common_prototypes = np.load(
            os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "common_prototypes.npy"
            )
        )
        X_dm = compute_X_dm(
            superset,
    ...
    if local_prototypes:
        X_dm_local = compute_X_dm_local(
            superset, tract_name, distance_func=distance_func, nb_points=nb_points
        )
        feature_list.append(X_dm_local)
        print(
            "----> Added local dissimilarity matrix of size (%s, %s)"
            % (X_dm_local.shape)
        )
