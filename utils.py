...
def bundle2roi_distance(bundle, roi_mask, distance="euclidean"):
    ...
    """
    data = np.asanyarray(roi_mask.dataobj)
...
def streamlines_idx(
    ...
    if (D > warning_threshold).any():
        print(
            "WARNING (streamlines_idx()): for %s streamlines D > 1.0e-1 !!"
            % (D > warning_threshold).sum()
        )
    # print(D)
...
def compute_superset(
    ...
    distance = partial(parallel_distance_computation, distance=distance_func)
    true_tract = np.array(true_tract, dtype=object)
...
def compute_kdtree_and_dr_tractogram(
    tractogram, num_prototypes=40, distance_func=bundles_distances_mam, nb_points=20
):
    ...
    dm_tractogram, prototype_idx = compute_dissimilarity(
        tractogram, distance, num_prototypes, prototype_policy="sff", verbose=False
    )
    prototypes = tractogram[prototype_idx]
    print("Building the KD-tree of tractogram.")
    kdt = KDTree(dm_tractogram)
    print(
        "Time spent to compute the DR of the tractogram: %.2f seconds"
        % (time.time() - t0)
    )
