...
import pickle
from utils import (
    compute_kdtree_and_dr_tractogram,
    compute_superset,
    streamlines_idx,
    save_trk,
)
...
def compute_X_y_train(tract_name, moving_tractogram_fname, example_fname):
    ...
    superset_idx = compute_superset(
        tract, kdt, prototypes, k=2000, distance_func=distance_func, nb_points=nb_points
    )
    superset = moving_tractogram[superset_idx]
    exID = ntpath.basename(moving_tractogram_fname)[4:10]

    print("Computing X_train.")
    X_train = compute_feature_matrix(
        superset, tract_name, distance_func=distance_func, nb_points=nb_points
    )

    print("Computing y_train.")
    y_train = np.zeros(len(superset))
    tract_idx = streamlines_idx(
        tract, kdt, prototypes, distance_func=distance_func, nb_points=nb_points
    )
...
def compute_union_superset_idx(
    kdt, prototypes, ex_dir_tract, distance_func=bundles_distances_mam, nb_points=20
):
    ...
    for i in range(th):
        ...
        tract = nib.streamlines.load(example_fname)
        tract = tract.streamlines
        superset_idx_test = compute_superset(
            tract,
            kdt,
            prototypes,
            k=2000,
    ...
    union_superset_idx = list(
        OrderedDict.fromkeys(union_superset_idx)
    )  # removes duplicates
...
def classifyber(moving_tractograms_dir, static_tractogram_fname, ex_dir_tract):
    ...
    for i in range(nt):
        moving_tractogram_fname = "%s/%s" % (
            moving_tractograms_dir,
            moving_tractograms[i],
        )
        example_fname = "%s/%s" % (ex_dir_tract, examples[i])
        X_tmp, y_tmp = compute_X_y_train(
            tract_name, moving_tractogram_fname, example_fname
        )
    ...
    union_superset_idx = compute_union_superset_idx(
        kdt, prototypes, ex_dir_tract, distance_func=distance_func, nb_points=nb_points
    )
    static_superset = static_tractogram[union_superset_idx]
    X_test = compute_feature_matrix(
        static_superset, tract_name, distance_func=distance_func, nb_points=nb_points
    )
    ...
    clf = LogisticRegression(
        class_weight=cw, random_state=42, solver="sag", max_iter=max_iter
    )

    t0 = time.time()
    clf.fit(X_train, y_train)
    print(
        "---->Time to fit X_train of size (%s, %s) = %.2f seconds"
    ...
    print(
        "---->Time to predict X_test of size (%s, %s) = %.2f seconds"
        % (X_test.shape[0], X_test.shape[1], time.time() - t1)
    )
    estimated_tract_idx = np.where(y_pred > 0)[0]
    estimated_tract = static_tractogram[union_superset_idx[estimated_tract_idx]]
    np.save(
        "estimated_idx_%s.npy" % tract_name, union_superset_idx[estimated_tract_idx]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-static", nargs="?", const=1, default="", help="The static tractogram filename"
    )
    parser.add_argument(
        "-ex_dir",
        nargs="?",
    ...
    parser.add_argument(
        "-list", nargs="?", const=1, default="", help="The tract name list file .txt"
    )
    parser.add_argument(
        "-out_dir", nargs="?", const=1, default="default", help="The output directory"
    )
    ...
    for tract_name in tract_name_list:
        ...
        print(
            "Time to compute classification of tract %s = %i minutes"
            % (tract_name, (time.time() - t1) / 60)
        )
    ...
    print(
        "Total time elapsed for the classification of all the tracts = %i minutes"
        % ((time.time() - t0) / 60)
    )
    sys.exit()
