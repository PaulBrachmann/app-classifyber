...
def parallel_distance_computation(
    A, B, distance, n_jobs=-1, granularity=2, verbose=False, job_size_min=1000
):
    ...
    if (len(A) > job_size_min) and joblib_available and (n_jobs != 1):
        ...
        if n_jobs > 1:
            tmp = np.linspace(0, len(A), granularity * n_jobs + 1).astype(int)
        ...
        dissimilarity_matrix = np.vstack(
            Parallel(n_jobs=n_jobs, verbose=verbose)(
                delayed(distance)(A[start:stop], B) for start, stop in chunks
            )
        )
    else:
